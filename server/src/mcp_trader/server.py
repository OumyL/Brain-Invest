...
**Data Reliability:** {data_source_emoji} 
   🟢 Tiingo (Primary) | 🟡 Binance (Fallback) 
"""
...
**Status:** Diagnostic completed with formatting limitations
...
**Note:** This error does not necessarily indicate system failure
"""
#screening tool
@mcp.tool()
...
**Disclaimer:** This analysis is based on available financial data and should not be considered as financial advice. Always conduct your own research and consult with financial professionals before making investment decisions.
"""
...
**Note:** Si certaines données sont manquantes, c'est probablement dû aux limites API temporaires. Réessayez dans quelques minutes.

**Disclaimer:** Cette recommandation est basée sur l'analyse des données disponibles et ne constitue pas un conseil financier.
"""
...
**Disclaimer:** Quick recommendation based on technical analysis. Not financial advice.
"""
...
**Note:** System issue - use alternative analysis methods
 ...
 **Note:** Temporary system issue - use alternative analysis tools

 **Error Time:** {datetime.now().strftime('%Y-%m-%d %H:%M')}
"""
def calculate_unified_score(trend_data, fundamental_data=None, weights=None):
    """Calcul unifié du score pour tous les outils."""
    if weights is None:
        weights = {'technical': 0.7, 'fundamental': 0.3}
    
    # Score technique basé sur la même logique que get_trading_signals
    technical_score = 50  # Base neutre
    
    # Tendance (40 points max)
    if trend_data.get("above_200sma"):
        technical_score += 20  # Long terme
    else:
        technical_score -= 20
        
    if trend_data.get("above_50sma"):
        technical_score += 10  # Moyen terme
    else:
        technical_score -= 10
        
    if trend_data.get("20_50_bullish"):
        technical_score += 10  # Court terme
    else:
        technical_score -= 10
        
    # MACD (20 points max)
    if trend_data.get("macd_bullish"):
        technical_score += 20
    else:
        technical_score -= 20
    ...
    if 40 <= rsi <= 60:
        technical_score += 5  # Zone neutre favorable
    elif rsi > 70:
        technical_score -= 15  # Surachat
    elif rsi < 30:
        technical_score += 10  # Survente = opportunité
    
    # Limiter entre 0-100
    technical_score = max(0, min(100, technical_score))
    
    # Score fondamental si disponible
    if fundamental_data:
        fundamental_score = fundamental_data.get('composite_score', 50)
