...
class TechnicalAnalysis:
    ...
    def get_trading_signals(df: pd.DataFrame) -> dict[str, Any]:
        ...
        if trend_status.get("20_50_bullish"):
            bull_signals += 2
            signal_details.append("20/50 SMA Bullish Cross")
        else: 
            bear_signals += 2
        ...
        if 40 <= rsi <= 60:
            ...
            bull_signals += 2 #Oversold can be bullish for reversal
            ...
            bull_signals += 1
            signal_details.append(f"RSI Below Neutral ({rsi:.1f})")
            total_signals += 1
            
        # MACD signals - AJOUT MANQUANT
        if trend_status.get("macd_bullish"):
            bull_signals += 2
            signal_details.append("MACD Bullish Cross")
        else:
            bear_signals += 2
            signal_details.append("MACD Bearish Signal")
        total_signals += 2
    ...
    # Calculate overall signal strength - LOGIQUE CORRIGÉE
        if total_signals > 0:
        ...
        # SEUILS CORRIGÉS pour éviter les "Strong BUY" inappropriés
        if bull_percentage >= 75:
            overall_signal = "Strong BUY"
            signal_emoji = "🚀"
        elif bull_percentage >= 65:  # RELEVÉ de 60 à 65
            overall_signal = "BUY"
            signal_emoji = "📈"
        elif bull_percentage >= 35:  # BAISSÉ de 40 à 35
            overall_signal = "HOLD"
            signal_emoji = "⚖️"
        elif bull_percentage >= 25:  # BAISSÉ de 30 à 25
            overall_signal = "SELL"
            signal_emoji = "📉"
        else:
